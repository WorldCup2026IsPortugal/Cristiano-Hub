...
spawn(function() 
    ...
    wait(.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>hoangthienlongdznhatvutru!!!!!<Color=/>"):Display()
    wait(.14)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>Trigon is better than Fluxus!!<Color=/>"):Display()
    wait(.24)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>Feauture Exploit will be dead :<<Color=/>"):Display()
    wait(.29)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>Channel : No Channel <Color=/>"):Display()
    wait(.36)
end)

local OrionLib = loadstring(game:HttpGet(('https://raw.githubusercontent.com/shlexware/Orion/main/source')))()
local Window = OrionLib:MakeWindow({Name = "Cristiano Hub", HidePremium = false, IntroText = "Orion Library", SaveConfig = true, ConfigFolder = "Cristiano Hub"})
...
OrionLib:MakeNotification({
	Name = "Cristiano Hub",
	Content = "Best Player Ever? Cristiano Ronaldo",
	Image = "rbxassetid://4483345998",
	Time = 25
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "Cristiano Hub(Noitification)",
